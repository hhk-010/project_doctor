...
class _SearchListViewState extends State<SearchListView> {
  ...
  Widget build(BuildContext context) {
    return StreamProvider<QuerySnapshot>.value(
      ...
      initialData: null,
      child: BaseScaffold(
        title: 'view_search_result_list.search_result'.tr(),
        isAppbar: true,
...
class _ResultListState extends State<ResultList> {
  ...
  getNameScore(String name1) {
    String name2 = ProfileSearchData.name;
    List name1list = [];
    List name2list = [];
  ...
  void initState() {
    super.initState();
    DatabaseService.province = ProfileSearchData.province;
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (streamData != null) {
      for (var doc in streamData.docs) {
        ...
        //} else {
        if (ProfileSearchData.speciality == doc.data()['speciality']) {
          newList.add(
            ProfileSearchListData(
                id: doc.id,
                name: doc.data()['name'],
...
class SearchListTile extends StatefulWidget {
  final ProfileSearchListData searchResultList;
